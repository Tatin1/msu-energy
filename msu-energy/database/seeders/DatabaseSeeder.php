...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach($names as $n){
            $b = \App\Models\Building::create(['code'=>$n['code'],'name'=>$n['name'],'is_online'=>rand(0,10)>1]);
            // create 2 meters per building
            for($i=1;$i<=2;$i++){
                $m = $b->meters()->create(['meter_code'=>$b->code.'-'.$i,'label'=>$b->code.'-'.$i]);
                // create hourly readings (12 values)
                for($h=0;$h<12;$h++){
                    $m->readings()->create([
        ...
        // default tariff
        \App\Models\Tariff::create(['name'=>'default','rate'=>12.00]);
    }
